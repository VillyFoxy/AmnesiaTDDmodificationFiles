...
string UserStringOutput, idCount, dataNetworkGlobal, iSetString, dataNetwork2;
float idCountGlobal;
...
void cLuxScriptHandler::InitScriptFunctions()
{
	...
	AddFunc("float GetPlayerRotation()", (float *)GetPlayerRotation);
	...
	AddFunc("int GetAnimationNetwork()", (int *)GetAnimationNetwork);
	...
	AddFunc("void setPacket(float PosX, float PosY, float PosZ)", (void *)setPacket);
	AddFunc("void setObjPacket(int objA, int objB, int objC)", (void *)setObjPacket);
	AddFunc("void Controls()", (void *)Controls);
	AddFunc("int GetAnimation()", (int *)GetAnimation);
	...
	AddFunc("void SetDynamicSnowTexture(string &in asName)", (void *)SetDynamicSnowTexture);
	...
	AddFunc("int GetObjNetwork(int i)", (int *)GetObjNetwork);
	...
	AddFunc("void SetTrueFalse(bool trueFalse)", (void *)SetTrueFalse);
	AddFunc("bool GetTrueFalse()", (bool *)GetTrueFalse);
	...
	AddFunc("void SetSide(int side)", (void *)SetSide);
	AddFunc("int GetSide()", (int *)GetSide);
	...
	AddFunc("int GetPlayerRot()", (int *)GetPlayerRot);
	AddFunc("void SetPlayerRot(int sideNetwork)", (void *)SetPlayerRot);
	...
	AddFunc("void startConnection()", (void *)startConnection);
	...
	AddFunc("void npcRotation(string& asName, float afAcc, float afGoalSpeed, float playerX, float playerY, float npcX, float npcY, bool abResetSpeed)", (void *)npcRotation);
	...
	AddFunc("float getNetwork(int i)", (float *)getNetwork);
...
void __stdcall cLuxScriptHandler::SetDynamicSnowTexture(string& asName)
{
...
/* float __stdcall cLuxScriptHandler::GetCrouch()
{
	return gpBase->mpPlayer->GetCharacterBody()->GetSize().y;
} */
...
int __stdcall cLuxScriptHandler::GetPlayerRot()
{
	return gpBase->mpPlayer->iGetSideNetwork();
}

//-----------------------------------------------------------------------

void __stdcall cLuxScriptHandler::SetTrueFalse(bool trueFalse)
{
	iTrueFalse = trueFalse;
}

//-----------------------------------------------------------------------

bool __stdcall cLuxScriptHandler::GetTrueFalse()
{
	return iTrueFalse;
}
...
void __stdcall cLuxScriptHandler::SetPlayerRot(int sideNetwork)
{
	gpBase->mpPlayer->iSetSideNetwork(sideNetwork);
}

//-----------------------------------------------------------------------

void __stdcall cLuxScriptHandler::SetSide(int side)
{
	gpBase->mpPlayer->SetSideId(side);
}

//-----------------------------------------------------------------------

int __stdcall cLuxScriptHandler::GetSide()
{
	return gpBase->mpPlayer->GetSideId();
}
...
/* float __stdcall cLuxScriptHandler::getNetwork(int i)
{
	return gpBase->mpPlayer->getNetworkId(i);
}

void __stdcall cLuxScriptHandler::setNetwork(float PosX, float PosY, float PosZ)
{
	gpBase->mpPlayer->setNetworkId(PosX, PosY, PosZ);
} */

float __stdcall cLuxScriptHandler::GetPlayerRotation()
{
	return gpBase->mpPlayer->GetCharacterBody()->GetCameraPosAdd().y;
	// return gpBase->mpPlayer->GetCharacterBody()->GetSize().y;
}

void __stdcall cLuxScriptHandler::Controls()
{
	IdAnimation = 0;
	if (GetAsyncKeyState(87) && !GetAsyncKeyState(16))
	{
		IdAnimation = 1;
	}
	if (GetAsyncKeyState(87) && GetAsyncKeyState(16))
	...
	if (GetAsyncKeyState(83) && GetAsyncKeyState(16))
	{
		IdAnimation = 3;
	}
	if (GetAsyncKeyState(83) && !GetAsyncKeyState(87) && !GetAsyncKeyState(16))
	{
		IdAnimation = 4;
	}
	if (GetAsyncKeyState(68) && !GetAsyncKeyState(16) && !GetAsyncKeyState(83) && !GetAsyncKeyState(87))
	{
		IdAnimation = 5;
	}
	if (GetAsyncKeyState(65) && !GetAsyncKeyState(16) && !GetAsyncKeyState(83) && !GetAsyncKeyState(87))
	{
		IdAnimation = 6;
	}
	...
	if (gpBase->mpPlayer->GetCharacterBody()->GetSize().y <= 1.5)
	{
		IdAnimation = 8;
	}
	if (GetAsyncKeyState(65) && GetAsyncKeyState(16) && !GetAsyncKeyState(83) && !GetAsyncKeyState(87))
	...
	if (gpBase->mpPlayer->GetCharacterBody()->GetSize().y <= 1.5 && GetAsyncKeyState(87))
	{
		IdAnimation = 11;
	}
	if (gpBase->mpPlayer->GetCharacterBody()->GetSize().y <= 1.5 && GetAsyncKeyState(83) && !GetAsyncKeyState(87))
	{
		IdAnimation = 12;
	}
	if (gpBase->mpPlayer->GetCharacterBody()->GetSize().y <= 1.5 && GetAsyncKeyState(68) && !GetAsyncKeyState(87) && !GetAsyncKeyState(83))
	{
		IdAnimation = 13;
	}
	if (gpBase->mpPlayer->GetCharacterBody()->GetSize().y <= 1.5 && GetAsyncKeyState(65) && !GetAsyncKeyState(87) && !GetAsyncKeyState(83))
	{
		IdAnimation = 14;
	}
}

int __stdcall cLuxScriptHandler::GetAnimation()
{
	return IdAnimation;
}

int __stdcall cLuxScriptHandler::GetAnimationNetwork()
{
	return IdAnimationNetwork;
}

int __stdcall cLuxScriptHandler::GetObjNetwork(int i)
{
	return iObjNetwork[i];
}

void __stdcall cLuxScriptHandler::setPacket(float PosX, float PosY, float PosZ)
{
	std::string msg = "", xStr = "", yStr = "", zStr = "";
	...
	send(ConnectionSocket, (char*)&packettype, sizeof(Packet), NULL);
	send(ConnectionSocket, (char*)&msg_size, sizeof(int), NULL);
	send(ConnectionSocket, msg.c_str(), msg_size, NULL);
}

void __stdcall cLuxScriptHandler::setObjPacket(int objA, int objB, int objC)
{
	std::string msg = "", aStr = "", bStr = "", cStr = "";
	std::stringstream ss, ss2, ss3;
	ss << objA;
	ss >> aStr;
	ss2 << objB;
...
bool __stdcall cLuxScriptHandler::ProcessPacket(Packet& packettype)
{
	switch (packettype)
	{
	case Msg:
	{
		int msg_size;
		string dataNetwork = "";
		recv(ConnectionSocket, (char*)&msg_size, sizeof(int), NULL);
		char* msg = new char[msg_size + 1];
		msg[msg_size] = '\0';
		recv(ConnectionSocket, msg, msg_size, NULL);
		dataNetwork = msg;
		auto posEndCoords = dataNetwork.find(",");
		auto posEndCoords2 = dataNetwork.find(":");
		if (posEndCoords != std::string::npos)
		{
			iGetNetworkX = atof(dataNetwork.substr(0, posEndCoords).c_str());
			iGetNetworkY = atof(dataNetwork.substr(posEndCoords + 1, posEndCoords2).c_str());
			iGetNetworkZ = atoi(dataNetwork.substr(posEndCoords2 + 1).c_str());
		}
		delete[] msg;
		break;
	}
	case Count:
	{
		...
		recv(ConnectionSocket, msg, msg_size, NULL);
		dataNetwork2 = msg;
		auto posEndCoords = dataNetwork2.find(",");
		auto posEndCoords2 = dataNetwork2.find(":");
		if (posEndCoords != std::string::npos)
		{
			iObjNetwork[0] = atoi(dataNetwork2.substr(0, posEndCoords).c_str());
			iObjNetwork[1] = atoi(dataNetwork2.substr(posEndCoords + 1, posEndCoords2).c_str());
			iObjNetwork[2] = atoi(dataNetwork2.substr(posEndCoords2 + 1).c_str());
		}
		delete[] msg;
		break;
	}
...
void __stdcall cLuxScriptHandler::ClientHandler()
{
	Packet packettype;
	while (true)
	{
		recv(ConnectionSocket, (char*)&packettype, sizeof(Packet), NULL);
...
void __stdcall cLuxScriptHandler::startConnection()
{
	string msg1, ip;
	int portClient;
	WSAData wsaData;
	...
	ifstream file("network.txt", ios::in | ios::binary);
	while (true)
	{
		getline(file, str);
		if (!file.eof())
		{
			...
			for (int r = 0; r < strings; r++)
			{
				if (r == 1)
				{
					file.getline(mass[1], len - 1, ch);
					portClient = atoi(mass[1]);
				}
			}
		}
		else break;
...
void __stdcall cLuxScriptHandler::npcRotation(string& asName, float afAcc, float afGoalSpeed, float playerX, float playerY, float npcX, float npcY, bool abResetSpeed)
{
	BEGIN_SET_PROPERTY(eLuxEntityType_Prop, -1)

		iLuxProp *pProp = ToProp(pEntity);
	pProp->SetFullGameSave(true);
	// int angle = ((atan2(playerX - pProp->GetBody(0)->GetWorldPosition().x, playerY - pProp->GetBody(0)->GetWorldPosition().y) - 1) * 58) / 3.14159265;

	// otladka = (pProp->GetBody(0)->GetLocalMatrix().m[0][0] * 500) / 3.14159265;
	// otladka2 = ((atan2(playerX - pProp->GetBody(0)->GetWorldPosition().x, playerY - pProp->GetBody(0)->GetWorldPosition().y) - 2) * 30) / 3.14159265;
	
	// pProp->RotateAtSpeedModified(afAcc, afGoalSpeed, cVector3f(0, angle, 0), abResetSpeed, false, 0, 0);
	pProp->GetBody(0)->GetLocalMatrix().m[1][0] = afAcc;
	pProp->GetBody(0)->GetLocalMatrix().m[0][2] = afGoalSpeed;
	pProp->GetBody(0)->GetLocalMatrix().m[1][2] = playerX;
